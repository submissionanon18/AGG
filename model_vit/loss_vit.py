...
class Loss_vit(torch.nn.Module):
    ...
    def __init__(self, cfg,  lambda_ssim=1.0,lambda_dir_cls=1.0,lambda_contra_ssim=1.0,lambda_trg=0.0,lambda_self_cls = 0.0):
        super().__init__()
        ...
        self.lambdas = dict(
            ...
            lambda_self_cls = lambda_self_cls
        )
    ...
    def forward(self, outputs, source,out_prev=None,use_dir=True,target=None,frac_cont=1.0):
        ...
        if self.lambdas['lambda_self_cls'] > 0:
            losses['loss_self_cls'] = self.calculate_self_cls_loss(outputs, source)
            loss_G += losses['loss_self_cls'] * self.lambdas['lambda_self_cls']
            losses_val['loss_self_cls'] = losses['loss_self_cls'].item()
        if use_dir:
    ...
    def calculate_dir_cls_loss(self, outputs,out_prev):
        ...
        return loss
    def calculate_self_cls_loss(self, outputs,source):
        loss = 0.0
        for a, b  in zip(outputs, source):  # avoid memory limitations
            a = self.global_transform(a).unsqueeze(0).to(device)
            b = self.global_transform(b).unsqueeze(0).to(device)
            cls_token = self.extractor.get_feature_from_input(a)[-1][0, 0, :]
            with torch.no_grad():
